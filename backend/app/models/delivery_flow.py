...
class CandidateDeliveryResponse(BaseModel):
    ...
    session_id: str = Field(..., description="Active PlaybackSession UUID")
    stream_url: str = Field(..., description="Authoritative Go streamer URL (http://127.0.0.1:8088/stream/...)")
    player_url: str = Field(..., description="Web player URL (/api/media/session/{session_id}/player)")
    elapsed_seconds: float = Field(..., description="Time taken to deliver and verify media")
